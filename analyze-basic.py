...
class SubmarinerAnalyzer:
    ...
    def detect_blocking_type(self):
        ...
        for gw, _cluster in [(gateway1, "cluster1"), (gateway2, "cluster2")]:
    ...
    def analyze_network_topology(self):
        ...
        for cluster in ['cluster1', 'cluster2']:
            ...
            if not cluster_ips:
                ...
                self._print("           (Network topology analysis requires pod hostIP data from gather output)")
                continue
    ...
    def check_ovn_routing(self):
        ...
        for cluster in ['cluster1', 'cluster2']:
            ...
            if not remote_cidrs:
                ...
                self._print("           (OVN route validation requires remote subnet data from Gateway CR or Endpoint)")
                continue
    ...
    def _check_globalnet_snat(self, nft_content, cluster, node_name):
        ...
        all_snat_rules = []
        for _chain_name, chain_content in snat_chains:
            snat_rules = re.findall(r'snat (?:ip )?to (\S+)', chain_content)
    ...
    def _analyze_segment2_evidence(self, segments):
        ...
        self._print("        Note: SNAT/DNAT counters are cumulative (all GlobalNet traffic since nftables loaded)")

        # Check cluster2 → cluster1 path
        if c2_snat > 0 and c1_tcpdump > 0 and c1_dnat == 0:
            self._print("        cluster2 → cluster1: Packets reach GW1 but not DNAT rule")
            self._print("          → It seems that post-decapsulation routing is broken")
            self._print("          → Packets decapsulated but not routed to ovn-k8s-mp0")

        # Check cluster1 → cluster2 path
        if c1_snat > 0 and c2_tcpdump > 0 and c2_dnat == 0:
            self._print("        cluster1 → cluster2: Packets reach GW2 but not DNAT rule")
            self._print("          → It seems that post-decapsulation routing is broken")
            self._print("          → Packets decapsulated but not routed to ovn-k8s-mp0")
        ...
        if cable_driver == 'libreswan':
            ...
            self._print("      → Also see 'IPsec Diagnostic Data' section above for detailed analysis")
        ...
        if icmp_analysis:
            ...
            if c1_sending_healthchecks and c2_sending_healthchecks:
                ...
                if c1_unreachable:
                    self._print("          cluster1: Sending Unreachable instead of Echo Requests")
                    self._print("          → cluster1 cannot route to remote cluster health check IP")
                    # For VXLAN, check ARP resolution
                    if cable_driver == 'vxlan':
                        c1_arp = c1_data.get('arp_count', 0)
                        if c1_arp == 0:
                            self._print("          → VXLAN: No ARP packets detected - gateway may not be able to resolve remote gateway MAC")
                if c2_unreachable:
                    self._print("          cluster2: Sending Unreachable instead of Echo Requests")
                    self._print("          → cluster2 cannot route to remote cluster health check IP")
                    # For VXLAN, check ARP resolution
                    if cable_driver == 'vxlan':
                ...
                self._print("        ✗ Neither cluster sending health checks")
                self._print("        → Tunnel may not be established OR tcpdump missed packets")
            else:
                # Asymmetric: one sending, one not
                if c1_sending_healthchecks and not c2_sending_healthchecks:
                    self._print("        ⚠ Asymmetric health checks:")
                    self._print("          cluster1: Sending Echo Requests ✓")
                    self._print("          cluster2: NOT sending Echo Requests ✗")
                    if not c2_unreachable:
                        self._print("          → cluster2: Possible tcpdump timing issue or local routing problem")
                elif c2_sending_healthchecks and not c1_sending_healthchecks:
                    self._print("        ⚠ Asymmetric health checks:")
                    self._print("          cluster2: Sending Echo Requests ✓")
                    self._print("          cluster1: NOT sending Echo Requests ✗")
                    if not c1_unreachable:
                        self._print("          → cluster1: Possible tcpdump timing issue or local routing problem")
        else:
            # Fallback to basic tcpdump count analysis
            if c1_tcpdump > 0 and c2_tcpdump > 0:
                self._print("        ⚠ Bidirectional traffic detected (packet types unknown)")
                self._print("        → Cannot determine if health checks are working without ICMP analysis")
            elif c1_tcpdump == 0 and c2_tcpdump == 0:
                self._print("        ✗ No tunnel traffic detected")
            else:
                self._print("        ⚠ Asymmetric traffic pattern")
        ...
        if icmp_analysis and (icmp_analysis.get('cluster1', {}).get('unreachable_egress', 0) > 0 or
            ...
            self._print("        Investigate local routing on cluster(s) with ICMP Unreachable")
            if cable_driver == 'vxlan':
                ...
                if c1_arp == 0 or c2_arp == 0:
                    self._print("        VXLAN: Check ARP resolution (no ARP packets detected)")
            self._print("        Check routes to remote GlobalNet health check IP")
        else:
            self._print("        Contact Submariner community with this diagnostic tarball")
            self._print("        Include packet analysis showing ICMP types and directions")
    ...
    def _analyze_ipsec_pcap_contents(self):
        ...
        tcpdump_dir = os.path.join(self.diagnostics_dir, "tcpdump")
        if not os.path.exists(tcpdump_dir):
            self._print("        ℹ No tcpdump files available for analysis")
            return None
        ...
        if not pcap_files:
            self._print("        ℹ No pcap files available")
            return None
            ...
            try:
                ...
                if total_esp > 0:
                    ...
                    for line in esp_lines[:3]:
                        ...
                        sample_count += 1  # noqa: SIM113
                        if sample_count >= 3:
                            break
                else:
                    self._print("          ℹ No ESP packets detected")
    ...
    def _validate_tcpdump_consistency(self, cluster_results):
        ...
        if missing_packets:
            ...
            self._print("        → Tcpdump started after some packets were sent, or")
            self._print("        → Capture window missed early traffic")
    ...
    def _analyze_segment1_evidence(self, segments):
        ...
        if c1_segment in ['SEGMENT_1_ALL_WORKERS', 'SEGMENT_1_SPECIFIC_WORKERS']:
            ...
            self._print("        Evidence collected: Table 150 routes, IP rules, nftables")
            self._print("        → See 'OVN-K Host Networking Verification' section above for details")

            if cni1 == 'OVNKubernetes':
                self._print(f"\n        {Colors.BOLD}Submariner configuration appears correct{Colors.ENDC}")
                self._print("        It seems that this is an infrastructure or OVN-K platform issue")
                self._print("        Recommend contacting Submariner community with this diagnostic tarball:")
                self._print("          https://github.com/submariner-io/submariner/discussions")
    ...
    def correlate_tunnel_failure(self):
        ...
        if c1_segment == 'SEGMENT_2_TUNNEL':
            ...
            self._print("      GW@cluster1 → GW@cluster2: error")
            self._print("      All RouteAgents@cluster1: error")
            ...
            self._print("      GW@cluster1 → GW@cluster2: connected")
            self._print("      All RouteAgents@cluster1: error")
        ...
        if c2_segment == 'SEGMENT_2_TUNNEL':
            ...
            self._print("      GW@cluster2 → GW@cluster1: error")
            self._print("      All RouteAgents@cluster2: error")
            ...
            self._print("      GW@cluster2 → GW@cluster1: connected")
            self._print("      All RouteAgents@cluster2: error")
        ...
        if c1_dnat_packets == 0 and c2_dnat_packets > 100:
            ...
            self._print("    cluster1: receiving 0 health check packets")
            ...
            if not c2_has_outbound or c2_tcpdump_total < 10:
                ...
                if c2_unreachable:
                    ...
                    self._print("    Root cause: Missing routes or routing configuration issue on cluster2")
                    ...
                    self._print("    Recommendation: Check cluster2 routing tables and OVN configuration")
                ...
                if c1_segment == 'SEGMENT_1_ALL_WORKERS' or c1_segment == 'SEGMENT_1_SPECIFIC_WORKERS':
                    ...
                    self._print("    Segment analysis: Local routing (Segment 1) appears broken")
                    self._print("    Evidence collected: Table 150 routes, IP rules, OVN configuration")
                    self._print("    → See 'OVN-K Host Networking Verification' section above for details")
                    ...
                    self._print("    This appears to be a health check mechanism issue")
                    self._print("    Actual datapath may be functional despite Gateway error status")
            ...
            self._print("    cluster2: receiving 0 health check packets")
            self._print(f"\n  {Colors.BOLD}Evidence Analysis:{Colors.ENDC}")
            ...
            self._print("    Both clusters receiving 0 health check packets")
            self._print("    This suggests a bidirectional routing or datapath issue")
